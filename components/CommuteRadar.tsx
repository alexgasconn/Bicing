...
import { MapPin, Navigation, Clock, Activity, ArrowRight, Ban, CheckCircle, AlertTriangle, Mountain, X } from 'lucide-react';
...
const CommuteRadar: React.FC<CommuteRadarProps> = ({
  ...
  const analysis = useMemo(() => {
    ...
    if (!startStation || !endStation) {
        status = 'red';
        message = "No hi ha estacions properes als punts seleccionats.";
    } else {
        ...
        if (bikes === 0 || slots === 0) {
            ...
            status = 'yellow';
            message = "Disponibilitat justa. Risc de quedar-se sense.";
        }
    }
  ...
  return (
    <div className="absolute top-20 left-4 right-4 md:left-auto md:right-4 md:w-96 bg-white rounded-2xl shadow-2xl z-[2000] overflow-hidden border border-slate-200 animate-in slide-in-from-top-4 fade-in">
                ...
                <div className="flex items-center gap-3">
                    ...
                    <div>
                        ...
                        <div className="text-sm font-bold text-slate-800 truncate max-w-[180px]">
                            {origin ? `Lat: ${origin.lat.toFixed(4)}, Lng: ${origin.lng.toFixed(4)}` : 'Toca al mapa...'}
                        </div>
                    </div>
                </div>
                {selectionMode === 'origin' && <div className="text-xs font-bold text-blue-600 animate-pulse">Seleccionant...</div>}
            </div>
            ...
            >
                <div className="flex items-center gap-3">
                    ...
                    <div>
                        ...
                        <div className="text-sm font-bold text-slate-800 truncate max-w-[180px]">
                            {destination ? `Lat: ${destination.lat.toFixed(4)}, Lng: ${destination.lng.toFixed(4)}` : 'Toca al mapa...'}
                        </div>
                    </div>
                </div>
                 {selectionMode === 'destination' && <div className="text-xs font-bold text-red-600 animate-pulse">Seleccionant...</div>}
            </div>
        </div>
        ...
        {analysis ? (
            <div className="p-4 space-y-4">
                ...
                <div className={`p-3 rounded-lg flex items-center gap-3 ${
                    ...
                    {analysis.status === 'green' ? <CheckCircle size={24} /> : 
                     analysis.status === 'yellow' ? <AlertTriangle size={24} /> : <Ban size={24} />}
                ...
                <div className="space-y-2 text-sm">
                    <div className="flex justify-between items-center p-2 bg-blue-50 rounded border border-blue-100">
                        <span className="font-bold text-blue-800">A: {analysis.startStation.name}</span>
                        <span className="font-black bg-white px-2 rounded text-blue-600 border border-blue-200">{analysis.startStation.free_bikes} Bicis</span>
                    </div>
                    <div className="flex justify-center">
                        <ArrowRight className="text-slate-300" />
                    </div>
                     <div className="flex justify-between items-center p-2 bg-red-50 rounded border border-red-100">
                        <span className="font-bold text-red-800">B: {analysis.endStation.name}</span>
                        <span className="font-black bg-white px-2 rounded text-red-600 border border-red-200">{analysis.endStation.empty_slots} Llocs</span>
                    </div>
                </div>
            </div>
        ) : (
